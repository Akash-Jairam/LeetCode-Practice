class NumArray {
    ...
    public SegmentTreeNode buildTree(int[] nums, int start, int end){
        if(start > end)
            ...
            SegmentTreeNode toRet = new SegmentTreeNode(start, end);
            if(start == end){
                toRet.sum = nums[start];
            } else{
                int mid = start + (end - start) / 2;
                toRet.left = buildTree(nums, start, mid);
                toRet.right = buildTree(nums, mid + 1, end);
                toRet.sum = toRet.left.sum + toRet.right.sum;
            }
